...
\begin{frame}
\frametitle{Ecuaciones de Kolmogorov}
\framesubtitle{Ecuación Hacia Adelante (Forward Equation)}
...
En espacios de estado infinitos (ej. $S = \mathbb{N}$), surge un fenómeno conocido como \textbf{explosión}.
...
\vspace{0.2cm}
Si por el contrario, $P(\zeta = \infty) = 1$, decimos que la cadena es \textbf{regular} (estable). En un sistema regular, la matriz $Q$ determina completamente la evolución en todo tiempo $t \ge 0$.

\end{frame}

% Criterio para Procesos de Nacimiento 
\begin{frame}
\frametitle{Condición de Explosión}
\framesubtitle{El criterio exacto para Procesos de Nacimiento Puro}

Para entender cuándo ocurre la explosión, analizamos un proceso de nacimiento puro (donde las transiciones solo van de $n \to n+1$) con tasas de nacimiento $\lambda_n$.

El tiempo total esperado para dar infinitos saltos es la suma de los tiempos esperados en cada estado:
\[ E[\zeta] = \sum_{n=0}^\infty E[T_n] = \sum_{n=0}^\infty \frac{1}{\lambda_n} \]
...
% Comparación: n vs n^2 
\begin{frame}
\frametitle{Condición de Explosión}
\framesubtitle{Ejemplo clásico: Crecimiento lineal vs Cuadrático}
...
\[ \sum_{n=1}^\infty \frac{1}{n^2} = 1 + \frac{1}{4} + \frac{1}{9} + \dots = \frac{\pi^2}{6} \approx 1.645 \]
Esta serie converge. \textbf{El sistema explota} en un tiempo promedio de $\approx 1.645$ unidades de tiempo.
